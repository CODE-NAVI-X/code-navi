...
“用户”保留给真实账号或明确的本地个人资料能力，不用学习画像冒充用户中心。账号可选学生/教师两种身份；教师可创建班级并生成 8 位抗混淆邀请码，学生凭码加入；教师可管理班级成员名单（查看成员、查看学生邮箱、设置私有教学备注、移除学生），学生端严格保持隐私保护（不展示邮箱、不展示备注、无移除权限）。轻量班级管理不含复杂 LMS 与全量学情聚合。
...
除可执行代码练习外，Practice 开始提供不运行真实代码的“结构与框架练习”，面向已掌握基础语法的学生。该类练习通过服务端确定性规则判题，并按机器学习主题供学生选择，当前覆盖 CNN、RNN、Transformer、线性模型、树模型、优化算法和聚类算法等；它不调用 Piston、不安装深度学习依赖，也不表示模型已经训练或推理成功。可验收行为见 [Structure and Framework Practice Spec](../specs/structure-framework-practice.md)。
