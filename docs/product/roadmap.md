...
| 结构与框架练习 | 前后端最小实现完成 | 静态练习服务、Compiler API、Practice 页面入口和两种题型已实现；后端测试与前端 lint/build 通过，尚未做浏览器联调和持久化 |
...
在不扩大 Piston 执行范围的前提下，为已经掌握基础语法的学生增加不运行真实模型代码的结构与框架练习。第一版只做静态排序和框架填空，判题由服务端规则完成；CNN 示例只用于讲解结构逻辑，不执行真实训练。后续切片见 [结构与框架练习实施计划](../plans/structure-framework-practice-rollout.md)。

完成条件：

1. Practice 页面提供独立进入的结构与框架练习模式；
2. 列表、提交、反馈和错误状态按 Spec 在本地验证完成；
3. 静态练习不调用 Piston、不泄漏标准答案，也不创建错误的工作区活动；
4. 需要持久化时再接入安全结果摘要，并遵守现有 PracticeOutcome 隐私边界。
