...
输入只来自当前会话已保存的科研画像、用户明确选择的论文来源、A 的 `ReproductionPipeline` 只读适配视图和用户提交的实验记录。输出包含五个各 20 分的维度、实际得分、当前可评分上限、结构总上限 100、证据与信息范围、待核验项，以及用户可接受、跳过或完成的改进任务。证据不足的维度不评分。
