...
- 版本：`0.2.0`
- 用途：通过可恢复的规则会话收集科研需求，并在五个字段完整后生成结构化研究简报和规则研究计划。
...
输出固定包含：`session_id`、五字段 `state`、按固定顺序的 `missing_fields`、下一题及三个选项、`turns`。只有全部字段存在时才提供 `research_brief`、`research_plan` 并将 `completed` 设为 `true`。
...
- 可访问的信息源：仅本次 API 请求和本应用 SQLite 会话记录；不访问网络、论文库、MCP、文件系统或外部模型。学生端仅在浏览器本地保存 `session_id`，不保存密钥。
...
- 本版本不调用 LLM，因此不存在模型超时或 API Key 依赖；所有问题、推荐项和研究计划均由 `rules.py` 固定规则生成。
...
测试覆盖创建会话、固定三个选项、自由输入、初始描述、会话恢复、五字段完成简报和规则研究计划、未完成会话降级、输入校验和 404。前端可使用 `npm run lint` 和 `npm run build` 验证类型与页面构建。
