...
**现状更正（v2 评审）**：`PaperBlueprintSection.section` 现为六段中文枚举 `["引言","相关工作","方法","实验","讨论","结论"]`（`conversation_schemas.py:733`），按需生成、不落库。v2 `schema_version` 升级为 `"paper-blueprint.v2"`，枚举改为固定五段，顺序即严格输出顺序：

1. `摘要`（abstract，写作目标：从画像+计划规则提炼 ≤200 字结构化摘要骨架）
2. `介绍`（introduction）
3. `文献综述`（related_work）
4. `方法`（method）
5. `实验`（experiments）

沿用中文枚举值与现有代码风格一致；原"引言"内容并入"介绍"；"摘要"段为新增（写作目标：从画像+计划规则提炼 ≤200 字结构化摘要骨架，超出 200 字校验失败）；原"讨论"段的可信边界/风险内容并入"实验"段 `missing_evidence`，原"结论"段并入"方法"段 `writing_goal` 的收束要点——**讨论/结论不再作为独立段**（评审要求聚焦五段）。每段 `writing_goal`、`evidence_references`、`missing_evidence`、`forbidden_claims` 语义不变；prompt 中亦不得设问"对象与场景"。
...
六条固定为：
- ① **研究问题与假设可复述性**：0分（未保存画像与目标）、1分（仅有主题或非疑问形式）、2分（画像与Pipeline中均有清晰可复述的明确研究假设/问题）；
- ② **方法可执行性（步骤完整、变量可操作）**：0分（无步骤）、1分（步骤泛化缺少参数）、2分（≥2个结构化步骤且包含学习率/batch_size/优化器/超参数等可操作变量说明）；
- ③ **数据可得性（公开链接与许可）**：0分（无数据记录）、1分（仅有URL或仅有许可或跨条目分散）、2分（同一条目或同一个结构化 DatasetRef 中同时具备可核验公开获取链接与明确开源/使用许可）；
- ④ **指标与统计方法正确性（对照标准目录）**：0分（无指标条目）、1分（指标未命中标准指标目录，标记为待核验 to_verify）、2分（指标或 MetricSpec 明确命中服务端标准指标目录）；
- ⑤ **计算资源与时间可行性**：0分（无资源记录）、1分（缺少量化硬件/时间，或方案需求超出用户预算如需 24GB 显存但用户仅有 8GB）、2分（用户硬件与方案需求均量化且方案所需显存/内存 ≤ 用户可用资源，且时间预算 ≥ 方案耗时）；
- ⑥ **结果核验路径（baseline 与预期区间）**：0分（无基线与实验）、1分（缺结构化基线、预期非双边界区间、或缺用户实验结果）、2分（同时具备结构化 baseline、严格双边界数值预期区间如 `[0.80, 0.85]` 或 `81.5% ± 0.5%`，以及用户提交的非失败状态数值结果证据；禁止写“闭环/完成/成功”等过度宣称）。

总分 12，`total_score` 字段随响应返回。前端逐条结构化展示，不得合并为纯文本。
...
4. `submission-readiness` 等下游消费者按 v2 总分口径消费评估记录（12 分制，展示当前记录完整度 X/12，具体准备度阈值待产品裁定，系统不擅自设定 12/12 或其他通过/失败门槛），v1 历史值不参与新阈值计算，读侧遇到 v1 标注“历史口径”。
