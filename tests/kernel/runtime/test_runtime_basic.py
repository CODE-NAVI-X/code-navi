...
def test_runtime_config_priority_is_explicit_then_agent_then_runtime() -> None:
    ...
    explicit = runtime.run(
        agent_default, RuntimeRequest("go", run_id="explicit"), config=KernelConfig(max_steps=1)
    )
...
def _registry_for(run_id: str, names: tuple[str, ...], calls: list[object]):
    ...
    for name in names:
        registry.register(
            ...
            lambda args, context, tool_name=name: (
                calls.append((tool_name, dict(args), context.run_scope)) or args["value"].upper()
            ),
        )
    registry.freeze()
...
def test_runtime_tool_loop_uses_requested_tools_factory_and_dispatcher() -> None:
    ...
    provider = MockProvider(
        [
            ProviderResult(
                Message(
                    "assistant", (ContentBlock("tool_use", {"tool_call": tool_call.to_json()}),)
                )
            ),
    ...
    assert [
        event.type for event in result.events if event.type in {"tool_called", "tool_returned"}
...
def test_runtime_rejects_declared_tools_without_factory() -> None:
    ...
    with pytest.raises(ValueError, match="dispatcher_factory"):
        runtime.run(
            AgentSpec("tool_agent", "Uses tools.", "Use tools.", ("lookup",)), RuntimeRequest("go")
        )


@pytest.mark.parametrize(
    "declared, returned",
    [(("lookup",), ("lookup", "extra")), (("first", "second"), ("second", "first"))],
)
def test_runtime_rejects_factory_tool_sets_that_do_not_match_spec(
    declared: tuple[str, ...], returned: tuple[str, ...]
) -> None:
    def factory(run_id: str, names: tuple[str, ...]):
    ...
    with pytest.raises(ValueError, match="names and order"):
        runtime.run(
            AgentSpec("tool_agent", "Uses tools.", "Use tools.", declared),
            RuntimeRequest("go", run_id="bad-tools"),
        )
