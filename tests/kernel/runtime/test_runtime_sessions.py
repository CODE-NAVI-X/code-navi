...
def test_runtime_persists_event_only_session_logs(tmp_path: Path) -> None:
    runtime = AgentRuntime(MockProvider([ProviderResult(_assistant("saved"))]), session_dir=tmp_path)
    result = runtime.run(
    ...
    for forbidden in ("PermissionGrant", "destructive_tool_names", "workspace_roots", "allowed_permissions"):
...
@pytest.mark.parametrize("session_id, run_id", [("../escape", "run"), ("session", "../escape"), ("session/child", "run"), ("session", "run\\child")])
def test_runtime_rejects_session_path_traversal(tmp_path: Path, session_id: str, run_id: str) -> None:
    runtime = AgentRuntime(MockProvider([]), session_dir=tmp_path)
    with pytest.raises(ValueError, match="safe path segment"):
        runtime.run(AgentSpec("helper", "Helps.", "Help."), RuntimeRequest("go", session_id=session_id, run_id=run_id))
