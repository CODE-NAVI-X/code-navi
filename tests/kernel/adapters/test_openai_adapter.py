...
def status_error(status: int) -> openai.APIStatusError:
    ...
    return openai.APIStatusError(
        f"status {status}", response=response, body={"error": "recorded"}
    )
...
def test_request_translation_is_stateless_non_strict_and_sdk_retry_free() -> None:
    ...
    adapter = OpenAIResponsesAdapter(
        "explicit-model", client=client, max_output_tokens=64
    )
    tool = ProviderTool(
        "lookup",
        "Look up a value.",
...
def test_official_sdk_serializes_recorded_request_and_parses_response() -> None:
    ...
    adapter = OpenAIResponsesAdapter(
        "recorded-model", client=sdk_client, max_output_tokens=64
    )
    tool = ProviderTool(
        "lookup",
        "Look up a value.",
...
def test_retryable_status_mapping(status: int) -> None:
    adapter = OpenAIResponsesAdapter(
        "explicit-model", client=FakeClient([status_error(status)])
    )
...
def test_response_validation_error_is_fatal() -> None:
    ...
    adapter = OpenAIResponsesAdapter(
        "recorded-model", client=FakeClient([error])
    )
