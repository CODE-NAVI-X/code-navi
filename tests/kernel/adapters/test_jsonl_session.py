...
def test_load_strictly_rejects_malformed_or_unordered_jsonl(tmp_path: Path, contents: str) -> None:
...
def test_save_load_resume_matches_uninterrupted_without_duplicate_compression(
    ...
    uninterrupted = run(
        MockProvider([ProviderResult(tool_message()), ProviderResult(final_message)]),
        Dispatcher(),
        [],
        KernelConfig(max_steps=2, max_tool_calls=2),
        prior_events=initial,
        context_policy=TailWithSummary(UnitCounter(), Summarizer(), summary_budget_tokens=1),
...
def test_resume_requires_a_fresh_permission_grant(tmp_path: Path) -> None:
        ...
        MockProvider(
            [
                ProviderResult(
                    Message(
                        "assistant",
                        (ContentBlock("tool_use", {"tool_call": first_call.to_json()}),),
                    )
                )
            ]
        ),
        registry.bind(
            PermissionGrant("permission-run", frozenset({ToolPermission.EXECUTE})),
        ...
        MockProvider(
            [
                ProviderResult(
                    Message(
                        "assistant",
                        (ContentBlock("tool_use", {"tool_call": second_call.to_json()}),),
                    )
                ),
    ...
    returned = [
        item.payload["tool_result"] for item in resumed.events if item.type == "tool_returned"
    ]
