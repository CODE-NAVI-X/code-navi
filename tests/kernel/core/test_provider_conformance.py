    ...
    for block in result.message.content:
        if block.type == "text":
            output.append(
                SimpleNamespace(
                    type="message",
                    content=(SimpleNamespace(type="output_text", text=block.data["text"]),),
                )
            )
...
def build_recorded_openai(script: Sequence[ScriptItem]) -> OpenAIResponsesAdapter:
    ...
    native_script = [
        native_response(item) if isinstance(item, ProviderResult) else item for item in script
    ]
    return OpenAIResponsesAdapter("recorded-model", client=RecordedClient(native_script))
...
def recorded_rate_limit() -> openai.RateLimitError:
    ...
    return openai.RateLimitError("rate limited", response=response, body={"error": "recorded"})


_provider_cases = [ProviderCase("mock", build_mock, lambda: RetryableProviderError("rate limited"))]
...
def tool_message(*calls: ToolCall) -> Message:
    return Message(
        "assistant",
        tuple(ContentBlock("tool_use", {"tool_call": call.to_json()}) for call in calls),
    )
