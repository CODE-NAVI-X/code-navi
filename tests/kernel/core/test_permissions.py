...
def test_read_is_the_only_default_allowed_permission() -> None:
    ...
    result = dispatcher(registry, "read-run").dispatch(ToolCall("tc-read", "read_local", {}))
...
def test_non_read_permissions_are_deny_by_default(permission: ToolPermission) -> None:
    ...
    allowed = dispatcher(registry, "allowed", permissions={permission}).dispatch(
        ToolCall("tc-2", name, {})
    )
...
def test_composed_permissions_require_all_flags() -> None:
    ...
    result = dispatcher(registry, "partial", permissions={ToolPermission.NETWORK}).dispatch(
        ToolCall("tc-1", "remote_private_read", {})
    )
...
def test_destructive_requires_flag_and_specific_tool_name() -> None:
    ...
    no_flag = dispatcher(registry, "no-flag", destructive_names={"delete_file"}).dispatch(
        ToolCall("tc-1", "delete_file", {})
    )
    no_name = dispatcher(registry, "no-name", permissions={ToolPermission.DESTRUCTIVE}).dispatch(
        ToolCall("tc-2", "delete_file", {})
    )
    allowed = dispatcher(
        registry,
        "allowed",
...
def test_grant_does_not_leak_through_long_lived_registry() -> None:
    ...
    first = dispatcher(registry, "run-one", permissions={ToolPermission.EXECUTE}).dispatch(
        ToolCall("tc-1", "execute", {})
    )
    second = dispatcher(registry, "run-two").dispatch(ToolCall("tc-2", "execute", {}))
...
def test_write_is_allowed_only_inside_workspace(tmp_path: Path) -> None:
    ...
    traversal = bound.dispatch(ToolCall("tc-3", "write_new_file", {"path": "../escape.txt"}))
...
def test_denial_is_tool_result_and_loop_continues_without_new_event_types() -> None:
    ...
    tool_message = Message("assistant", (ContentBlock("tool_use", {"tool_call": call.to_json()}),))
    registry = ToolRegistry()
    registry.register(spec_for("execute", {ToolPermission.EXECUTE}), lambda args, context: "no")
    result = run(
