...
def test_record_save_load_and_replay_three_steps_identically(tmp_path: Path) -> None:
    ...
    def policy() -> TailWithSummary:
        return TailWithSummary(UnitCounter(), ConstantSummarizer(), summary_budget_tokens=1)
    ...
    attempts = [event.payload["attempt"] for event in loaded if event.type == "provider_called"]
