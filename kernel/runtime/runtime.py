...
class AgentRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        if context_policy is not None and not callable(
            getattr(context_policy, "view", None)
        ):
    ...
    def run(
        ...
        *,
        ...
        if tuple(tool.name for tool in provider_tools) != agent.tool_names:
            raise ValueError(
                "dispatcher provider_tools() names and order must match tool_names"
            )
        ...
        effective_config = (
            config or agent.default_config or self._default_config or KernelConfig()
        )
        run_result = run(
            self._provider,
            dispatcher,
