...
export default function LearningPage(): JSX.Element {
  ...
  const practiceHref = buildPracticeHref({
    knowledgePoint: practiceTopic || undefined,
    knowledgePointId: practiceTopic ? buildKnowledgeId(practiceTopic) : undefined,
    sessionId: activeSessionId,
    currentSearchParams: searchParams,
  });
  const inactiveModeClass =
    "flex items-center gap-1.5 rounded-lg px-4 py-2 text-xs font-medium text-slate-500 transition-all hover:text-slate-700 dark:text-zinc-400 dark:hover:text-zinc-200";
  const activeModeClass =
    "flex items-center gap-1.5 rounded-lg bg-white px-4 py-2 text-xs font-semibold text-slate-900 shadow-2xs dark:bg-zinc-900 dark:text-zinc-100";
  ...
  return (
    <div className="mx-auto min-w-0 max-w-7xl px-4 py-5 sm:py-8">
      <div className="mb-4 flex flex-wrap justify-end gap-2">
        ...
        >
          <Code2 className="h-3.5 w-3.5 text-slate-500 dark:text-zinc-400" strokeWidth={1.5} />
          动手实践
        </Link>
        <Link
        ...
        <button
          ...
          className="app-button-secondary flex cursor-pointer items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium shadow-sm transition hover:bg-slate-50 hover:text-slate-900 dark:hover:bg-zinc-800 dark:hover:text-white"
        >
          <ClipboardList className="h-3.5 w-3.5 text-slate-500 dark:text-zinc-400" strokeWidth={1.5} />
          当前笔记
        </button>
        <Link
          href="/learning/portrait"
          className="app-button-secondary flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium shadow-sm transition hover:bg-slate-50 hover:text-slate-900 dark:hover:bg-zinc-800 dark:hover:text-white"
        >
      ...
      <div className="mb-4 max-w-full overflow-x-auto rounded-xl border border-slate-200/50 bg-slate-100/90 p-1 dark:border-zinc-700/40 dark:bg-zinc-800/80">
        <div className="flex min-w-max">
          <Link href="/learning/explore" className={pathname === "/learning/explore" ? activeModeClass : inactiveModeClass}>
            <Search className="h-3.5 w-3.5" strokeWidth={1.5} />
            探索方向
          </Link>
          <Link href="/learning" className={pathname === "/learning" ? activeModeClass : inactiveModeClass}>
          ...
          <button
            ...
            disabled={!practiceTopic || quizLoading}
            className={`${inactiveModeClass} cursor-pointer disabled:cursor-not-allowed disabled:opacity-50`}
          >
            {quizLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" strokeWidth={1.5} /> : <FileQuestion className="h-3.5 w-3.5" strokeWidth={1.5} />}
            理解检查
          </button>
          <Link href={practiceHref} className={inactiveModeClass}>
            <Code2 className="h-3.5 w-3.5" strokeWidth={1.5} />
            动手实践
          </Link>
          <Link href="/learning/portrait" className={inactiveModeClass}>
            <UserRound className="h-3.5 w-3.5" strokeWidth={1.5} />
            复盘
          </Link>
          <Link href="/learning/notebook" className={inactiveModeClass}>
