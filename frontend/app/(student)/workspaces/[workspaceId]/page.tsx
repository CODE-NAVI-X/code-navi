  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      <nav aria-label="面包屑导航" className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-zinc-400">
        ...
        <span>&rarr;</span>
        <span className="text-slate-900 dark:text-zinc-100 font-bold">工作区详情</span>
      </nav>
      <header className="app-card mt-5 rounded-2xl p-5 sm:p-7">
        ...
        <h1 className="mt-2 break-words text-2xl font-bold text-slate-950 dark:text-zinc-50">{workspace.title}</h1>
        {workspace.description && <p className="mt-2 break-words text-sm text-slate-600 dark:text-zinc-300">{workspace.description}</p>}
        <form onSubmit={handleCreateTask} className="mt-5 flex flex-col gap-2 sm:flex-row">
          ...
          <input
            ...
            maxLength={2000}
            className="app-input min-w-0 flex-1 rounded-xl px-4 py-3 text-sm focus:border-slate-900 focus:ring-2 focus:ring-slate-900/15 focus:outline-none"
          />
          <button
            type="submit"
            disabled={creatingTask || !goal.trim()}
            className="app-button-primary inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        <div className="app-card rounded-2xl p-5">
          ...
          {tasks.length === 0 ? (
            ...
            <div className="mt-4 space-y-2">
              {tasks.map((task) => (
                <Link key={task.id} href={`/tasks/${task.id}`} className="app-card-subtle block rounded-xl p-3 hover:bg-slate-50 dark:hover:bg-zinc-800">
                  <p className="break-words font-semibold text-slate-950 dark:text-zinc-50">{task.title}</p>
                  <p className="mt-1 line-clamp-2 break-words text-xs leading-5 text-slate-600 dark:text-zinc-300">{task.goal}</p>
                </Link>
              ))}
            </div>
          )}
...
function PageState({
  ...
  return (
    <main
      ...
      className="mx-auto flex min-h-64 w-full max-w-5xl flex-col items-center justify-center gap-3 px-4 text-center text-sm text-slate-600 dark:text-zinc-300"
    >
      {icon}
      <p>{text}</p>
      {action}
