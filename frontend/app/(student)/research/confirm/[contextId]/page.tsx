...
export default function ResearchContextConfirmationPage() {
  ...
  if (!context) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4 dark:bg-zinc-950">
        <div className="w-full max-w-lg rounded-2xl border border-rose-200 bg-white p-6 text-center dark:border-rose-900 dark:bg-zinc-900">
          <h1 className="text-base font-bold text-slate-900 dark:text-zinc-100">无法打开待确认上下文</h1>
          <p className="mt-2 text-sm text-rose-700 dark:text-rose-300">{error}</p>
          <button type="button" onClick={() => router.push("/learning")} className="mt-5 rounded-xl bg-slate-900 px-4 py-2 text-sm text-white dark:bg-zinc-100 dark:text-zinc-900">
            返回知识点学习
          </button>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900 dark:bg-zinc-950 dark:text-zinc-100 sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <section className="mt-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:p-8">
          <div className="flex items-start gap-3">
            <div className="rounded-xl bg-indigo-50 p-2.5 text-indigo-600 dark:bg-indigo-950 dark:text-indigo-300">
              <FileText className="h-5 w-5" />
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-indigo-600 dark:text-indigo-300">Learning → Research</p>
          ...
          <div className="mt-7 space-y-5">
            <label className="block text-sm font-semibold">
              研究主题
              <input value={topic} onChange={(event) => { setTopic(event.target.value); setSaved(false); }} maxLength={500} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-indigo-400 dark:border-zinc-700 dark:bg-zinc-950" />
            </label>
            <label className="block text-sm font-semibold">
              上下文摘要
              <textarea value={summary} onChange={(event) => { setSummary(event.target.value); setSaved(false); }} maxLength={12000} rows={5} className="mt-2 w-full resize-y rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm leading-6 outline-none focus:border-indigo-400 dark:border-zinc-700 dark:bg-zinc-950" />
            </label>
            {selectedContent.map((item, index) => (
              <div key={item.kind} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-zinc-800 dark:bg-zinc-950/60">
          ...
          {error && <p role="alert" className="mt-5 rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-950/30 dark:text-rose-300">{error}</p>}
          ...
          <div className="mt-7 flex flex-wrap justify-between gap-3 border-t border-slate-100 pt-5 dark:border-zinc-800">
            ...
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={() => void saveDraft()} disabled={invalid || saving || clearing || confirming} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-700">
                {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} 保存草稿
              </button>
              <button type="button" onClick={() => void confirmDraft()} disabled={invalid || saving || clearing || confirming} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">
