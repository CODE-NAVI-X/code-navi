...
import { Download, Link as LinkIcon, Maximize2, Minimize2, Network, SlidersHorizontal, X } from "lucide-react";
  ...
  return (
    <aside className="rounded-2xl border border-slate-700/80 bg-zinc-950/95 p-4 text-xs shadow-2xl backdrop-blur dark:border-zinc-700" aria-label="节点详情">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-teal-300">节点详情</p>
      ...
      <p className="mt-3 rounded-xl bg-zinc-900 px-3 py-2 text-[10px] leading-4 text-zinc-400">事实边界：{STATUS_LABEL[node.status]}。{node.status === "evidence" ? "仅代表已保存的来源元数据或摘要范围。" : "不代表未经保存来源验证的论文事实。"}</p>
      {node.sources.length > 0 ? (
        <div className="mt-3 space-y-2 border-t border-zinc-800 pt-3">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">来源（仅点击后在新标签页打开）</p>
      ...
      ) : <p className="mt-3 text-[10px] text-zinc-500">暂无外部来源；此节点仅来自已保存画像或规则计划。</p>}
    </aside>
  );
}
...
function ResearchMapNode({ data }: NodeProps<MindMapFlowNode>) {
  ...
  return (
    <div className={`rounded-2xl ${hierarchyClass}`} style={{ backgroundColor: colors.fill, borderColor: colors.stroke, color: colors.text, boxShadow: isRoot ? `0 0 0 4px ${colors.stroke}33` : undefined }}>
      ...
      <p className="mt-1 line-clamp-2 text-[10px] leading-4 opacity-85">{summary}</p>
  ...
  return (
    <div className="mt-4 rounded-2xl border border-zinc-800 bg-zinc-950 p-3 text-zinc-100 sm:p-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800 pb-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-teal-300">专注导图工作区</p>
          <p className="mt-1 text-[11px] text-zinc-400">仅布局已保存节点与后端 edges；筛选只隐藏视图，不增加关系或调用网络。</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-zinc-400"><SlidersHorizontal className="h-3.5 w-3.5" /> 状态筛选</span>
          {ALL_STATUSES.map((status) => <button key={status} type="button" onClick={() => toggleStatus(status)} aria-pressed={activeStatuses.includes(status)} className={`rounded-full border px-2 py-1 text-[10px] font-semibold transition ${activeStatuses.includes(status) ? STATUS_STYLE[status] : "border-zinc-700 bg-zinc-900 text-zinc-500"}`}>{STATUS_LABEL[status]}</button>)}
          <button type="button" onClick={fitGraph} className="rounded-lg border border-zinc-700 px-2 py-1 text-[10px] font-semibold text-zinc-200 hover:bg-zinc-800">适配画布</button>
          <button type="button" onClick={() => downloadSvg(mindmap)} className="inline-flex items-center gap-1 rounded-lg border border-teal-800 bg-teal-950/40 px-2 py-1 text-[10px] font-semibold text-teal-200 hover:bg-teal-950"><Download className="h-3.5 w-3.5" /> 导出 SVG</button>
          <button type="button" onClick={onCollapse} className="inline-flex items-center gap-1 rounded-lg border border-zinc-700 px-2 py-1 text-[10px] font-semibold text-zinc-200 hover:bg-zinc-800"><Minimize2 className="h-3.5 w-3.5" /> 收起</button>
        </div>
      </div>
      <div
      ...
      {!selectedNode ? <p className="mt-3 text-center text-[11px] text-zinc-500">点击节点可查看依据、事实边界与已保存来源；详情不会占用默认画布。</p> : null}
    </div>
  );
}
...
function MindMapSummary({ mindmap, onExpand }: { mindmap: ResearchMindMap; onExpand: () => void }) {
  ...
  const planLabel = plan?.status === "inference" ? "规则研究计划已形成" : "研究计划待完善";
  return (
    <div className="mt-4 rounded-2xl border border-zinc-800 bg-gradient-to-br from-zinc-950 to-slate-900 p-4 text-zinc-100 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-teal-300">Research overview</p>
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onExpand} className="inline-flex items-center gap-1.5 rounded-xl bg-teal-400 px-3 py-2 text-xs font-bold text-zinc-950 hover:bg-teal-300"><Maximize2 className="h-3.5 w-3.5" /> 展开科研导图</button>
          <button type="button" onClick={() => downloadSvg(mindmap)} className="inline-flex items-center gap-1.5 rounded-xl border border-zinc-700 px-3 py-2 text-xs font-semibold text-zinc-200 hover:bg-zinc-800"><Download className="h-3.5 w-3.5" /> 导出 SVG</button>
        </div>
      </div>
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        ].map(([label, value, status]) => <div key={String(label)} className="rounded-xl border border-zinc-800 bg-zinc-900/80 p-3"><p className="text-[10px] text-zinc-500">{label}</p><p className={`mt-1 text-lg font-bold ${STATUS_STYLE[status as ResearchMindMapNodeStatus]}`}>{value}</p></div>)}
      </div>
      <p className="mt-4 text-[11px] leading-5 text-zinc-400">导图只读取已保存 ResearchProfile、规则 ResearchPlan 与 EvidenceBundle；不联网、不调用模型、不读论文全文、不写文件。</p>
    </div>
  );
}
...
export function ResearchMindMapPanel({ mindmap }: { mindmap: ResearchMindMap }) {
  ...
  return (
    <section className="rounded-2xl border border-teal-200 bg-white p-4 shadow-sm dark:border-teal-900/70 dark:bg-zinc-900/80">
      ...
      <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50/70 p-3 text-[11px] leading-5 text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/20 dark:text-amber-200">此导图区分已确认、建议、待验证、来源证据和风险；只整理已保存状态，不联网、不调用模型。当前支持 SVG，PNG 尚未实现。</p>
      {expanded ? <MindMapRendererBoundary fallback={<MindMapFallback mindmap={mindmap} />}><InteractiveMindMap mindmap={mindmap} onCollapse={() => setExpanded(false)} /></MindMapRendererBoundary> : <MindMapSummary mindmap={mindmap} onExpand={() => setExpanded(true)} />}
