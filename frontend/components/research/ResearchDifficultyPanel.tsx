...
const MODE = { llm: "模型个性化建议", rules: "基础规则", rules_fallback: "模型失败后的规则降级" } as const;
...
export function ResearchDifficultyPanel({ analysis }: { analysis: TopicDifficultyAnalysis }) {
  return (
    <section className="rounded-2xl border border-orange-200 bg-white p-4 shadow-sm dark:border-orange-900/70 dark:bg-zinc-900/80">
      ...
      <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50/70 p-3 text-[11px] leading-5 text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/20 dark:text-amber-200"><AlertTriangle className="mr-1 inline h-3.5 w-3.5" /> {MODE[analysis.generation_mode]}：这是研究设计的风险与缺口提示，不是论文精读或实验结论；当前范围：{analysis.information_scope === "metadata_and_abstract_only" ? "已有元数据/摘要" : "科研画像与规则计划"}。</p>
