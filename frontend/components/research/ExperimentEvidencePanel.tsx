...
export function ExperimentEvidencePanel({ conversationId }: { conversationId: string }) {
  ...
  useEffect(() => {
    let active = true;
    void listExperimentEvidenceBundles(conversationId)
      .then((value) => { if (active) setBundles(value); })
  ...
  async function saveEvidence() {
    ...
    try {
      const saved = await createExperimentEvidenceBundle(conversationId, {
        ...
        goal,
        items: [{ category, classification, content }],
      });
  ...
  return (
    <section className="rounded-2xl border border-orange-200 bg-white p-4 shadow-sm dark:border-orange-900/70 dark:bg-zinc-900/80">
      <div className="flex items-center gap-3">
        <span className="rounded-xl bg-orange-100 p-2 text-orange-700 dark:bg-orange-950/50 dark:text-orange-300"><ClipboardCheck className="h-4 w-4" /></span>
        <div><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-orange-700 dark:text-orange-300">Experiment evidence</p><h2 className="mt-1 text-sm font-bold">实验结果证据包</h2></div>
      </div>
      ...
      <div className="mt-3 grid gap-2">
        <input value={experimentName} onChange={(event) => setExperimentName(event.target.value)} maxLength={500} placeholder="实验名称" className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs dark:border-zinc-700 dark:bg-zinc-950" />
        <input value={goal} onChange={(event) => setGoal(event.target.value)} maxLength={1000} placeholder="实验目标" className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs dark:border-zinc-700 dark:bg-zinc-950" />
        <div className="grid grid-cols-2 gap-2">
          <select value={category} onChange={(event) => setCategory(event.target.value as ExperimentEvidenceCategory)} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[11px] dark:border-zinc-700 dark:bg-zinc-950">{categoryLabels.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
          <select value={classification} onChange={(event) => setClassification(event.target.value as AnalysisClassification)} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[11px] dark:border-zinc-700 dark:bg-zinc-950">{Object.entries(classificationLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
        </div>
        <textarea value={content} onChange={(event) => setContent(event.target.value)} maxLength={4000} rows={4} placeholder="例如：指标、数值、失败原因、表格文本或待确认条件…" className="resize-y rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs leading-5 dark:border-zinc-700 dark:bg-zinc-950" />
      </div>
      <button type="button" onClick={() => void saveEvidence()} disabled={saving} className="mt-2 inline-flex items-center gap-1 rounded-lg border border-orange-200 px-2.5 py-1.5 text-[11px] font-semibold text-orange-800 disabled:opacity-50 dark:border-orange-900 dark:text-orange-300">{saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Plus className="h-3.5 w-3.5" />}保存结果记录</button>
      {error && <p role="alert" className="mt-2 whitespace-pre-wrap text-xs text-rose-600">{error}</p>}
      {loading ? <p className="mt-3 text-[11px] text-slate-500">正在恢复已保存的实验结果…</p> : bundles.length ? <div className="mt-3 space-y-2">{bundles.map((bundle) => <details key={bundle.bundle_id} className="rounded-lg border border-orange-100 p-2 dark:border-orange-950/70"><summary className="cursor-pointer text-xs font-semibold">{bundle.experiment_name.content} · {new Date(bundle.submitted_at).toLocaleString()}</summary><p className="mt-2 text-[11px]"><EntryLabel classification={bundle.goal.classification} />：{bundle.goal.content}</p>{bundle.items.map((item, index) => <p key={`${item.category}-${index}`} className="mt-1 text-[11px]"><EntryLabel classification={item.classification} />：{item.content}</p>)}<p className="mt-2 text-[10px] text-slate-500">证据范围：{bundle.provenance_note}</p></details>)}</div> : <p className="mt-3 text-[11px] text-slate-500">尚未保存实验结果。没有结果时仍可生成“待补充”的论文蓝图。</p>}
      <button type="button" onClick={() => void buildBlueprint()} disabled={buildingBlueprint} className="mt-4 inline-flex items-center gap-1 rounded-xl bg-slate-900 px-3 py-2 text-xs font-bold text-white disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900">{buildingBlueprint ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FileText className="h-3.5 w-3.5" />}我确认生成论文蓝图</button>
      {blueprint && <div className="mt-3 rounded-xl border border-orange-200 p-3 text-xs dark:border-orange-900"><p className="font-bold">{blueprint.candidate_titles[0]?.content}</p><p className="mt-1 text-[11px]">投稿就绪度：<EntryLabel classification={blueprint.submission_readiness.classification} /> · {blueprint.submission_readiness.content}</p>{blueprint.sections.map((section) => <details key={section.section} className="mt-2 rounded-lg bg-slate-50 p-2 dark:bg-zinc-950/50"><summary className="cursor-pointer font-semibold">{section.section}：{section.writing_goal.content}</summary><p className="mt-1 text-[11px]">可用证据：{section.evidence_references.length ? section.evidence_references.map((reference) => reference.label).join("；") : "暂无"}</p><p className="mt-1 text-[11px]">待补充：{section.missing_evidence.map((item) => item.content).join("；") || "无"}</p></details>)}<p className="mt-2 text-[10px] text-slate-500">{blueprint.provenance_note}</p></div>}
    </section>
  );
}
