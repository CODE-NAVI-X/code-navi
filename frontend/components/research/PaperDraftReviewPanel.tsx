...
function ReadinessItems({ items }: { items: SubmissionReadinessItem[] }) {
  return <ul className="mt-1 space-y-1">{items.map((item) => <li key={item.id} className="rounded bg-slate-50 p-2 dark:bg-zinc-950"><p>{item.message}</p><p className="mt-1 text-sm text-slate-500"><ClassificationBadge classification={item.classification} /> · 依据：{item.basis}</p></li>)}</ul>;
}
...
export function PaperDraftReviewPanel({ conversationId }: { conversationId: string }) {
  ...
  return <section className="rounded-2xl border border-violet-200 bg-white p-4 shadow-sm dark:border-violet-900/70 dark:bg-zinc-900/80">
    <div className="flex items-center gap-3"><span className="rounded-xl bg-violet-100 p-2 text-violet-700 dark:bg-violet-950/50 dark:text-violet-300"><FilePenLine className="h-4 w-4" /></span><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-violet-700 dark:text-violet-300">Local draft review</p><h2 className="mt-1 text-lg font-bold">论文初稿与结构化审稿</h2></div></div>
    <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-zinc-400">仅接受当前本地会话主动粘贴的 Markdown/纯文本。DOCX/PDF 导入、最终格式导出留待后续；不会联网、写入项目、自动投稿，也不能替代导师或同行评审。</p>
    {loading && <p className="mt-3 text-sm text-slate-500 dark:text-zinc-400">正在恢复本地初稿与修订记录…</p>}
    <details open className="mt-3 rounded-xl border border-violet-100 dark:border-violet-950">
      ...
      <div className="border-t border-violet-100/70 px-2 pb-3 dark:border-violet-950/60">
        <p className="mt-2 text-sm leading-6 text-slate-500">本地规则辅助，不代表满足任何会议或期刊要求；只在你保存档案和确认检查后使用，不会联网抓取投稿规则。</p>
        ...
        <button type="button" onClick={() => void persistSubmissionProfile()} disabled={busy} className="mt-2 inline-flex min-h-10 items-center gap-1 rounded-lg border border-violet-200 px-3 py-2 text-sm font-semibold text-violet-800 disabled:opacity-50 dark:border-violet-900 dark:text-violet-300"><FileCheck2 className="h-3.5 w-3.5" />保存投稿准备档案</button>
        {submissionProfile && <p className="mt-1 text-sm text-slate-500">已恢复本地档案 · 最近更新 {new Date(submissionProfile.updated_at).toLocaleString()}</p>}
      </div>
    </details>
    <details open className="mt-3 rounded-xl border border-violet-100 dark:border-violet-950">
    ...
    <button type="button" onClick={() => void submitDraft()} disabled={busy} className="mt-2 inline-flex min-h-10 items-center gap-1 rounded-lg border border-violet-200 px-3 py-2 text-sm font-semibold text-violet-800 disabled:opacity-50 dark:border-violet-900 dark:text-violet-300">{busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Clipboard className="h-3.5 w-3.5" />}提交本地初稿</button>
    {error &&
      (failure && retryAction ? (
    ...
    {draft && <div className="mt-4 rounded-xl border border-violet-100 p-3 text-base leading-7 dark:border-violet-950"><p className="font-semibold">已保存：{draft.title} · v{draft.version}</p><p className="mt-1 text-sm text-slate-500">{new Date(draft.created_at).toLocaleString()} · 仅限用户粘贴的本地会话文本</p><button type="button" onClick={() => void runReview()} disabled={busy} className="mt-2 inline-flex min-h-10 items-center gap-1 rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900"><RotateCcw className="h-3.5 w-3.5" />我确认生成结构化审稿建议</button></div>}
    {review && <div className="mt-3 space-y-2 text-base leading-7"><p className="text-sm text-slate-500">{review.generation_mode === "llm" ? "模型个性化解释（事实边界仍由规则控制）" : "基础规则审稿"}</p>{review.revision_tasks.map((task) => <div key={task.task_id} className="rounded-lg border border-slate-200 p-2 dark:border-zinc-700"><p className={`font-semibold ${severityClass[task.finding.severity]}`}>{severityLabel[task.finding.severity]} · {task.finding.section}</p><p className="mt-1">{task.finding.issue}</p><p className="mt-1 text-sm text-slate-500">依据：{task.finding.basis} · <ClassificationBadge classification={task.finding.classification} /></p><p className="mt-1">建议：{task.finding.recommended_action}</p><div className="mt-2 flex gap-2"><button type="button" onClick={() => void changeTask(task.task_id, "accepted")} disabled={busy || task.status === "accepted"} className="inline-flex min-h-10 items-center gap-1 text-emerald-700 disabled:opacity-50 dark:text-emerald-300"><Check className="h-3.5 w-3.5" />接受</button><button type="button" onClick={() => void changeTask(task.task_id, "skipped")} disabled={busy || task.status === "skipped"} className="inline-flex min-h-10 items-center gap-1 text-slate-500 disabled:opacity-50"><X className="h-3.5 w-3.5" />跳过</button><span className="text-sm text-slate-500">当前：{task.status}</span></div></div>)}<button type="button" onClick={() => void previewRevision()} disabled={busy || !review.revision_tasks.some((task) => task.status === "accepted")} className="inline-flex min-h-10 items-center gap-1 rounded-lg border border-violet-200 px-3 py-2 text-sm font-semibold text-violet-800 disabled:opacity-50 dark:border-violet-900 dark:text-violet-300">生成已接受任务的修订预览</button></div>}
    {review && review.revision_tasks.filter((task) => task.status === "accepted").map((task) => {
      ...
      return <div key={`suggestion-${task.task_id}`} className="mt-3 rounded-xl border border-sky-200 p-3 text-xs dark:border-sky-900">
        ...
        <p className="mt-1 text-sm text-slate-500">候选改写是建议，不自动覆盖原稿；仅用户接受后才创建新版本。</p>
        {!suggestion && <button type="button" onClick={() => void generateSuggestion(task.task_id)} disabled={busy} className="mt-2 inline-flex min-h-10 items-center gap-1 rounded-lg border border-sky-200 px-3 py-2 text-sm font-semibold text-sky-800 disabled:opacity-50 dark:border-sky-900 dark:text-sky-300"><Pencil className="h-3.5 w-3.5" />生成候选改写</button>}
        {suggestion && <div className="mt-2 space-y-2"><p className="text-sm text-slate-500">{suggestion.generation_mode === "llm" ? "模型个性化建议（规则已校验）" : "基础规则"}</p><p><strong>原文片段：</strong>{suggestion.original_excerpt}</p><p><strong>依据：</strong>{suggestion.basis} · <ClassificationBadge classification={suggestion.classification} /></p><p><strong>修改理由：</strong>{suggestion.rationale}</p><textarea value={editingSuggestionId === suggestion.suggestion_id ? editedCandidate : suggestion.candidate_text} onChange={(event) => { setEditingSuggestionId(suggestion.suggestion_id); setEditedCandidate(event.target.value); }} rows={6} className="w-full resize-y rounded border border-slate-200 bg-white p-2 text-base leading-7 dark:border-zinc-700 dark:bg-zinc-950" /><p className="text-sm text-slate-500">缺证据内容必须保留待验证占位；不会自动检索、写文件、安装或执行。</p><div className="flex gap-2"><button type="button" onClick={() => void applySuggestion(task.task_id, "accepted")} disabled={busy} className="min-h-10 text-emerald-700 dark:text-emerald-300">接受并创建版本</button><button type="button" onClick={() => void applySuggestion(task.task_id, "skipped")} disabled={busy} className="min-h-10 text-slate-500">跳过候选</button></div></div>}
      </div>;
    })}
      </div>
    </details>
    <details open className="mt-3 rounded-xl border border-violet-100 dark:border-violet-950">
      ...
      <div className="border-t border-violet-100/70 px-2 pb-2 dark:border-violet-950/60">
    {!revision && !readiness && <p className="mt-2 text-sm text-slate-500 dark:text-zinc-400">接受审稿任务并生成候选改写后，这里会展示修订预览；投稿前检查需你再次确认后才会执行。</p>}
    {revision && <div className="mt-3 rounded-xl border border-violet-200 p-3 text-base leading-7 dark:border-violet-900"><div className="flex items-center justify-between gap-2"><p className="font-semibold">修订预览 · v{revision.version}</p><button type="button" onClick={() => void copyRevision()} className="min-h-10 text-violet-700 dark:text-violet-300">复制文本</button></div><p className="mt-1 text-sm text-slate-500">原稿不会被覆盖；以下仅含已接受任务的建议，仍需人工核对。</p><details className="mt-2"><summary className="min-h-10 cursor-pointer py-2 font-semibold">查看变更摘要与差异</summary><ul className="mt-1 list-disc pl-4">{revision.change_summary.map((item) => <li key={item}>{item}</li>)}</ul><pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap rounded bg-slate-50 p-2 text-sm dark:bg-zinc-950">{revision.diff_preview}</pre></details><pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap rounded bg-slate-50 p-2 text-base leading-7 dark:bg-zinc-950">{revision.content}</pre><button type="button" onClick={() => void runSubmissionReadiness()} disabled={busy} className="mt-3 inline-flex min-h-10 items-center gap-1 rounded-lg border border-violet-200 px-3 py-2 text-sm font-semibold text-violet-800 disabled:opacity-50 dark:border-violet-900 dark:text-violet-300"><FileCheck2 className="h-3.5 w-3.5" />我确认执行投稿前检查</button></div>}
    ...
    {readiness && <div className="mt-3 rounded-xl border border-amber-200 p-3 text-base leading-7 dark:border-amber-900"><p className={`font-semibold ${readinessClass[readiness.readiness_status]}`}>投稿前检查：{readinessLabel[readiness.readiness_status]}</p><p className="mt-1 text-sm text-slate-500">只检查本地已保存的草稿、修订预览和证据状态；不联网、不投稿，也不代表可被接收。</p>{readiness.blockers.length > 0 && <details className="mt-2" open><summary className="min-h-10 cursor-pointer py-2 font-semibold text-rose-700 dark:text-rose-300">阻塞项（{readiness.blockers.length}）</summary><ReadinessItems items={readiness.blockers} /></details>}{readiness.warnings.length > 0 && <details className="mt-2"><summary className="min-h-10 cursor-pointer py-2 font-semibold">警告（{readiness.warnings.length}）</summary><ReadinessItems items={readiness.warnings} /></details>}<details className="mt-2"><summary className="min-h-10 cursor-pointer py-2 font-semibold">人工核验项（{readiness.manual_checks.length}）</summary><ReadinessItems items={readiness.manual_checks} /></details><button type="button" onClick={() => void exportAssistantPackage()} disabled={busy} className="mt-3 inline-flex min-h-10 items-center gap-1 rounded-lg border border-violet-200 px-3 py-2 text-sm font-semibold text-violet-800 disabled:opacity-50 dark:border-violet-900 dark:text-violet-300"><Download className="h-3.5 w-3.5" />我确认导出投稿前辅助包</button><p className="mt-1 text-sm text-slate-500">仅导出研究与计划摘要、投稿档案、检查清单、修订依据和已选引用摘要；不含初稿/修订稿全文，均待作者或导师核对，不是最终投稿格式。</p></div>}
    {drafts.length > 1 && <p className="mt-3 text-sm text-slate-500">已恢复 {drafts.length} 个本地初稿版本；当前展示最近版本。</p>}
      </div>
    </details>
  </section>;
}
