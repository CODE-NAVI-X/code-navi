...
当前可演示流程为：**对话澄清 → 规则研究计划 → 用户主动触发的受限学术检索 → 用户主动选择受限证据的引用占位 → 难点分析 → 实验方案 → 用户确认后代码草案 → 实验结果证据包 → 论文蓝图 → 用户粘贴初稿 → 结构化审稿 → 人工确认修订任务 → 修订预览 → 投稿前检查 → 用户主动受控导出 → 研究思维导图**。
...
- 引用占位与参考文献雏形只使用当前会话已保存的 EvidenceBundle 元数据与摘要范围；用户必须主动选择来源和目标章节。它不会联网补全、不自动插入或改写原稿；缺作者、年份、来源、DOI/arXiv 标识会标为 `to_verify`，参考条目只供作者和导师核对。
...
- 受控导出只在用户点击后返回 Markdown 与 JSON 辅助包，内容限于当前初稿、修订预览、审稿意见和检查清单；它隐藏 Key 与私有路径，不含会话历史、数据库、论文全文缓存或项目文件，也不是最终投稿格式。
