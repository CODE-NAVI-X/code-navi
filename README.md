...
Code Navi（智教码航）面向学生自主学习、代码练习和项目科研。当前仓库已经具备 Kernel 与 CLI、知识点学习、科研对话、模型生成研究计划和受限检索的本地闭环，以及基于本机 Piston 的 Python 练习原型。
...
科研页面位于 `/research`。它以可恢复的研究对话和用户主动触发的模型研究计划为基础；难点分析、实验方案和代码草案均在用户明确操作后调用模型，代码草案仅供预览。用户还可保存实验结果证据包，主动运行五维论文复现项目评估并管理改进任务，再生成论文蓝图；评估只衡量记录与证据完整度，蓝图只组织已保存证据与待验证缺口，都不是论文全文、复现成功或投稿结论。研究思维导图仅在用户点击后调用模型整理已保存状态，不联网；节点明确显示事实边界、来源链接与访问时间。

## 科研辅助本地演示闭环

Research Capability 当前可演示的内部流程为：**对话澄清 → 用户主动触发的模型研究计划 → 用户主动触发的受限学术检索 → 用户主动选择受限证据的引用占位与完整性检查 → 难点分析 → 实验方案 → 用户确认后代码草案 → 实验结果证据包 → 用户主动五维复现项目评估与改进任务 → 论文蓝图 → 用户粘贴初稿 → 结构化审稿 → 人工确认修订任务 → 修订预览 → 投稿前检查 → 用户主动受控导出 → 用户主动生成研究思维导图**。这条演示路径不定义产品入口顺序或 Task 生命周期。
...
- 五维复现项目评估只在用户点击后读取已保存画像、论文选择、最新 `ReproductionPipeline` 只读视图和实验记录；每维展示评分依据、信息范围、事实边界和待核验项。当前会话尚未生成 Pipeline 或证据不足时，对应维度保持 `not_evaluable` 且不计入当前可评分上限，不会用普通研究计划冒充复现方案。改进任务可接受、跳过或由用户标记完成，刷新后恢复；结果不表示复现成功、论文质量、可投稿或会被接收。
- 引用占位、完整性检查与可复制参考文献草案只使用当前会话已保存且由用户明确选择的 EvidenceBundle 元数据与摘要范围。用户点击后，规则检查会展示来源—章节映射、重复选择、未确认插入占位、元数据/信息范围缺口和人工核验清单；草案逐条关联原始来源，完整条目仍标为“非正式格式化参考文献”，复制文本不包含用户备注或会话内容。章节映射仅为 `inference`，缺口为 `to_verify`。它不会联网补全、读取全文、自动插入或改写原稿，结果不代表引用正确或论文可以投稿。
- 初稿只允许用户主动粘贴 Markdown/纯文本并保存在当前本地会话；结构化审稿、修订任务和投稿前检查均是建议，不是导师结论、同行评审结论或投稿资格判断。用户可保存目标投稿方向、匿名、篇幅/章节及伦理/数据要求组成的投稿准备档案；本地检查只读取该档案和已保存工件，不联网抓取 venue 规则，并会把缺失信息标为 `to_verify`。逐段候选改写只针对用户已接受任务；原文、候选、依据和事实边界都会展示，只有用户再次接受或手动编辑后才创建新版本，原稿和历史版本始终保留。
- 受控导出只在用户点击后返回 Markdown 与 JSON 投稿前辅助包，内容限于研究与计划摘要、投稿准备档案、检查清单、修订依据和已选引用摘要；它隐藏 Key 与私有路径，不含初稿或修订稿全文、会话历史、数据库、论文全文缓存或项目文件，也不是最终投稿格式，仍需作者或导师核对。
- 导图基于已保存的 ResearchProfile、规则 ResearchPlan 与 EvidenceBundle，用既有 `@xyflow/react` 和 Dagre 显示真实节点与后端边关系。默认是研究状态摘要卡，用户展开后进入专注工作区；节点详情只在点击后出现。它不联网、不读论文全文、不调用模型或写文件；当前支持真实 SVG 导出，PNG 需要额外且尚未验证稳定性的浏览器栅格化方案，保留为后续增强。
...
当前**尚未实现**论文全文下载与精读、完整 BibTeX、格式化参考文献、DOCX/PDF 导入、期刊模板适配、引用格式自动校验、ZIP/DOCX/PDF/LaTeX 最终投稿包导出、自动投稿、自动检索、自动实验、自动写入项目、自动安装依赖、自动执行代码，以及多 Agent/MCP；不能把建议或摘要范围外的信息表述为已验证事实。
