...
- 引用占位、完整性检查与参考文献雏形只使用当前会话已保存且由用户明确选择的 EvidenceBundle 元数据与摘要范围。用户点击后，规则检查会展示来源—章节映射、重复选择、未确认插入占位、元数据/信息范围缺口和人工核验清单；章节映射仅为 `inference`，缺口为 `to_verify`。它不会联网补全、读取全文、自动插入或改写原稿，结果不代表引用正确或论文可以投稿。
