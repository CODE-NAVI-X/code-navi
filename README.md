...
- 五维复现项目评估只在用户点击后读取已保存画像、论文选择、可用的 `ReproductionPipeline` 只读视图和实验记录；每维展示评分依据、信息范围、事实边界和待核验项。证据不足的维度保持 `not_evaluable` 且不计入当前可评分上限；A 的 Pipeline 合同未接入时不会用普通研究计划冒充。改进任务可接受、跳过或由用户标记完成，刷新后恢复；结果不表示复现成功、论文质量、可投稿或会被接收。
