...
当前可演示流程为：**对话澄清 → 规则研究计划 → 用户主动触发的受限学术检索 → 用户主动选择受限证据的引用占位与完整性检查 → 难点分析 → 实验方案 → 用户确认后代码草案 → 实验结果证据包 → 用户主动五维复现项目评估与改进任务 → 论文蓝图 → 用户粘贴初稿 → 结构化审稿 → 人工确认修订任务 → 修订预览 → 投稿前检查 → 用户主动受控导出 → 研究思维导图**。
