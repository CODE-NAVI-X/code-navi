...
科研页面位于 `/research`。它以可恢复的研究对话和规则研究计划为基础；难点分析和实验方案只有在用户点击确认后才调用模型，代码草案保持服务端固定模板并仅供预览。用户还可保存实验结果证据包，主动运行五维论文复现项目评估并管理改进任务，再生成论文蓝图；评估只衡量记录与证据完整度，蓝图只组织已保存证据与待验证缺口，都不是论文全文、复现成功或投稿结论。研究思维导图仅可视化已保存的画像、规则计划与证据包，不会触发模型或联网；节点明确显示事实边界、来源链接与访问时间。

## 科研辅助本地演示闭环

Research Capability 当前可演示的内部流程为：**对话澄清 → 规则研究计划 → 用户主动触发的受限学术检索 → 用户主动选择受限证据的引用占位与完整性检查 → 难点分析 → 实验方案 → 用户确认后代码草案 → 实验结果证据包 → 用户主动五维复现项目评估与改进任务 → 论文蓝图 → 用户粘贴初稿 → 结构化审稿 → 人工确认修订任务 → 修订预览 → 投稿前检查 → 用户主动受控导出 → 研究思维导图**。这条演示路径不定义产品入口顺序或 Task 生命周期。
