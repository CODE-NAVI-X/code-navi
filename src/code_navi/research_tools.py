...
def research_clarification_spec() -> ToolSpec:
    return ToolSpec(
        ...
        {"type": "object", "properties": {"topic": {"type": "string", "minLength": 2, "maxLength": 200}, "objective": {"type": "string", "maxLength": 300}, "research_object": {"type": "string", "maxLength": 300}}, "required": ["topic"], "additionalProperties": False},
        frozenset({ToolPermission.READ}),
    )

def research_clarification_handler(args: Mapping[str, Any], _context: ToolExecutionContext) -> dict[str, Any]:
    topic = str(args["topic"]).strip()
    objective = str(args.get("objective") or "待澄清")
    research_object = str(args.get("research_object") or "未限定")
    missing = [name for name, value in (("objective", args.get("objective")), ("research_object", args.get("research_object"))) if not value]
    return {"research_brief": {"topic": topic, "objective": objective, "research_object": research_object}, "missing_fields": missing, "next_step": "补充缺失字段后再生成研究计划。"}
