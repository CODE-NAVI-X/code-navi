...
_REPRODUCTION_CLAIM_PATTERN = re.compile(
    ...
    r"(?:复现|重现|再现|复刻)(?:验证|实验)?(?:已经完成|已完成|完成了|完成)|"
...
_NEGATION_PREFIX_PATTERN = re.compile(
    ...
    r")"
    r"(?:[“\"'「『（(【\[\s]|把|将|因为|因|由于|根据|依据|凭借|单凭|仅凭|由此|因此|轻易|直接|盲目|贸然|简单|提前|就|而|去|来|前述|目前|当前|已有|现有|这些|上述|结果|指标|数据|(?:轻易|直接|盲目|贸然)?(?:下|声称|断言|判定|认为|得出|下达|形成|确认|视作|视为|说明|定义|宣称|断定)){0,18}"
    r"[“\"'「『（(【\[\s]{0,2}$"
)
