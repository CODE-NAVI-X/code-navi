...
_NEGATION_PREFIX_PATTERN = re.compile(
    ...
    r"不代表|不等于|不等同于|不等同|不构成|并非|不是|不视为|不算作|不算|≠|!="
    r")"
    r")"
...
_UNGROUNDED_MASTERY_CLAIM_PATTERN = re.compile(
    ...
    r"(?:这)?(?:说明|证明|表明|代表|意味着)[^，。；\n]*(?:基本功|基础|功底|能力|水平|入口)[^，。；\n]*(?:扎实|过硬|很强|过人|具备|成型)|"
    r"(?:你的|你)[^，。；\n]*(?:基本功|基础|功底)[^，。；\n]*(?:扎实|过硬)|"
    r"(?:你)(?:目前|现在|已经|已)?(?:完全)?掌握(?:了|的)?[^，。；\n]*(?:基础|知识|概念|算法|流程|核心|原理)|"
...
def build_welcome_prompt(
    ...
    rules = (
        ...
        "2. 必须逐一向同学清晰展示上方 5 个推荐研究方向卡片的完整标题与描述；\n"
        "3. 鼓励同学从这 5 个卡片中选择一个感兴趣的方向，或者自由输入自己想做的其他方向；\n"
        "4. 事实边界红线与约束（绝对遵守）：\n"
        "   - 提及学习端输入时，必须严格表述为‘学习端记录显示你已学习...，当前进度记录为...’；\n"
        "   - 严禁输出‘你已掌握’、‘掌握的知识’、‘你已有的储备’、‘有看懂能力’、"
        "‘底子扎实’、‘基础扎实’、‘基础很棒’、‘很懂’、‘说明你’、‘具备’、"
        "‘评测跑过’、‘做过实验’或‘跑通过流程’；\n"
        "   - 禁止把学习信息夸大为研究结论、掌握度断言、实践经验断言或已跑通实验断言；\n"
        "   - 严禁由已学内容或学习进度推断用户的掌握度、理解程度、动手能力、"
        "实践经验、基本功、研究能力或研究入口"
        "（严禁声称用户‘已掌握’、‘已具备能力/入口’、‘说明基础扎实’、‘具备实践经验’、"
...
def build_need_clarification_prompt(
    ...
    rules = (
        ...
        "（严禁声称用户‘底子扎实’、‘基础扎实’、‘基础很棒’、‘很懂’、‘具备’、‘已掌握’）；\n"
        "   - 严禁声称或断言用户‘做过实验’、‘跑通过流程’、‘接触了基础数据集’或‘已完成复现’；\n"
        "   - 严禁在当前阶段下任何‘复现成功’、‘已完成复现’、‘跑通’或‘通过’的断言结论；\n"
