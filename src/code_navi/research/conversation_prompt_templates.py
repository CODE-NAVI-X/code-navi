...
8. 检索执行边界：严禁假称“检索正在进行中”、“检索在后台执行”、“结果稍后自动生成”
   或让用户“继续等待检索结果”；检索是系统同步动作，系统未触发检索时，
   应引导用户明确检索词或点击检索选项，绝不得编造后台异步等待状态。
"""
...
_REPRODUCTION_CLAIM_PATTERN = re.compile(
    ...
    # 4. 复现可靠 / 确认复现 / 复现成立 / 复现已可推进下一阶段
    #
    # 注意：这里**只保留带「复现」主体**的推进说法。裸的「可以进入下一阶段」是
    # 四阶段流程的正常阶段推进措辞（「选择计算粒度」「进入结果分析」），与复现成功
    # 无关——旧模式把两者混为一谈，于是用户选择计算粒度、输入「GPU 显存约 8GB」
    # 之后的正常回复都会被判成"复现成功断言"并整轮失败。
...
_GENERIC_STATE_NOUNS_PATTERN = re.compile(
    ...
    # 「检索词 / 关键词」是会话状态的通用角色名词，不是某个具体选择值：
    # 用户通过确认选项给出检索词后，模型回显「你已确认的检索词：<词>」属于
    # 可追溯的复述，不得被判成"编造用户选择"而整轮失败（浏览器事实 2026-09-11）。
    r"|检索词|关键词|检索方向|四组|三组|两组|一组|上述|这些|以下",
)
...
def _contains_unsupported_user_attribution(
    ...
    *,
    ...
    for m in matches:
        ...
        # 剥掉**前导**通用角色名词（「的四组检索词：」「研究方向」等），
        # 保留真正的选择值再做证据比对。
        # 注意只能剥前导：整串替换会顺手删掉值内部的「的」，
        # 让「图神经网络在引文网络上的节点分类」这种合法回声比对失败。
        cleaned = re.sub(
            r"^(?:你已?(?:确认|选择|选定)的)?(?:的)?"
            r"(?:四组|三组|两组|一组|几组|上述|这些|以下|上面)?"
            r"(?:检索词|关键词|检索方向|研究方向|研究主题|主题|课题|阶段|内容)"
            r"[：:、]?\s*",
            "",
            attributed,
        )
        cleaned = _strip_attribution_decorations(cleaned).strip().strip("：:，,、。 \t")
        if not cleaned:
            continue

        # Check if the attributed value appears in user evidence (substring match)
        if cleaned not in evidence_text:
            return True, cleaned
...
def build_welcome_prompt(
    ...
    rules = (
        ...
        "3. 推荐研究方向会由页面以可点击的卡片形式展示，因此不要在回复中逐条罗列卡片的"
        "标题或描述（严禁出现'【卡片 1】'这类提示词骨架字样）；只需用一句话说明"
        "已根据学习内容准备了几个可探索方向，并邀请同学点选卡片或直接说出想研究的方向；\n"
...
def build_need_clarification_prompt(
    ...
    rules = (
        ...
        "但尊重用户的探索意愿，不阻止；\n"
        "3. 澄清交互契约（结构化选项的唯一来源）：\n"
        "   - 本轮只提出**一个**主要待回答问题，禁止把多个互不相干的问题塞进同一轮；\n"
        "   - 紧接该问题下方给出 2~4 条简短、可直接选择的建议答案，每条独占一行，"
        "用「1. …」或「A. …」的列表写法，每条不超过 30 字；\n"
        "   - 有且只有这一组选项；不要重复罗列，也不要把同一道题的选项写两遍；\n"
        "   - 若确实没有可靠的建议答案，就只写问题本身，"
        "严禁为了凑数编造候选项或用户画像；\n"
        "   - 禁止在信息模糊时盲目宣称需求已完成；\n"
...
def build_search_guidance_prompt(
    ...
    rules = (
        ...
        "5. 严禁声称检索正在后台进行中或让用户等待；未执行时仅引导确认检索词。\n"
    )
    return {
