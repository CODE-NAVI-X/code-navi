...
- 版本：`0.3.0`
- 用途：提供 `research-mindmap.v1` 的节点、边、状态与来源数据；前端以“摘要卡 → 专注导图工作区”呈现交互式节点连线图和真实 SVG 导出。
...
- 不联网，不读取论文全文，不调用模型或 Tool；来源链接仅在用户点击时由浏览器新标签打开。
- 不写会话或用户项目，不安装依赖、不执行代码。摘要、展开、筛选、缩放和节点点击均不触发检索或写操作；SVG 只导出节点、连线、颜色和可见文字。

## 失败与规则降级

图谱渲染异常时前端显示本地节点详情清单；无计划/证据时摘要卡与图谱安全显示待验证节点。当前支持 SVG，PNG 因需额外栅格化/截图方案且未验证稳定性，作为后续增强而非已实现能力。
