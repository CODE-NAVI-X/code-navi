...
description: 对已保存的论文复现项目做六条证据完整度评估，并生成由用户控制的改进任务。
---
...
- 版本：`reproduction-project-evaluation.v2`
- 用途：在用户明确点击后，离线检查一个论文复现项目的记录与证据完整度；结果不是论文质量、复现成功或投稿结论。历史 v1 快照保持只读并标注“历史口径”。

## 输入与输出

- 输入：当前会话已保存的科研画像、用户明确选择的论文/来源、A 的 `ReproductionPipeline` 只读适配视图、用户主动提交的实验记录。
- 输出：六条固定准则（研究问题与假设、方法、数据、指标、资源与时间、结果核验路径），每条 0/1/2 分，`total_score` 满分 12，并附依据、证据引用和改进任务。v1 快照原样返回，不做 100→12 换算。

## 规则层与模型层边界

- 当前完全规则化，按显式条目是否存在、能否追溯和信息范围评分；新评估使用六条 12 分制，历史口径不参与新口径阈值。
