...
_legacy_structure_practice = StructurePracticeService()
...
async def list_legacy_structure_exercises() -> dict:
    """Return the frontend-compatible structure exercise catalogue."""
    return _legacy_structure_practice.list_exercises()


@router.post("/structure-exercises/{exercise_id}/submit", status_code=200)
async def submit_legacy_structure_exercise(exercise_id: str, payload: dict) -> dict:
    """Grade a structure exercise using the existing rule engine."""
    ...
    try:
        return _legacy_structure_practice.submit(exercise_id, payload["answer"], level=level)
...
async def evaluate_legacy_structure_exercise(exercise_id: str, payload: dict) -> dict:
    ...
    if level is not None and not isinstance(level, int):
        raise HTTPException(status_code=400, detail="level must be an integer")
    try:
        context = _legacy_structure_practice.exercise_public_context(exercise_id)
        rule_result = _legacy_structure_practice.submit(exercise_id, payload["answer"], level=level)
    except StructureExerciseNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except StructureExerciseValidationError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    try:
        ...
        return {"ai": {"status": "unavailable", "message": "AI 反馈暂时不可用；规则结论不受影响。"}}
    return {"ai": feedback.as_dict()}


@router.post("/structure-exercises/{exercise_id}/chat", status_code=200)
async def chat_legacy_structure_exercise(exercise_id: str, payload: dict) -> dict:
    ...
    if tutor is None:
        return {"ai": {"reply": ai_service.message, "strategy": "explanation", "blocked": False}}
    try:
        reply = tutor.chat(question, context, history, None)
    except AiEvaluationError:
        return {"ai": {"reply": "AI 引导暂时不可用。", "strategy": "explanation", "blocked": False}}
    return {"ai": reply}
...
@router.post(
    "/code-fill/evaluate",
    response_model=StructureEvaluationResponse,
    status_code=200,
)
async def evaluate_code_fill(
    request: CodeFillGradeRequest,
    principal: CurrentPrincipal | None = _opt_principal_dep,
    db: Session = _db_dependency,
) -> StructureEvaluationResponse:
    """Return AI feedback for one static structure/framework submission."""
    principal_id = principal.principal_id if principal else None
    owned_ids = get_owned_principal_ids(principal, db) if principal else None
    try:
        exercise, answer, rule_results = _practice_service.structure_evaluation_context(
            request,
            db,
            owner_principal_id=principal_id,
            owned_ids=owned_ids,
    ...
    evaluator = ai_service.evaluator
    if evaluator is None:
        return StructureEvaluationResponse(
            ai={
                "status": "disabled",
                "message": ai_service.message,
            }
        )
    try:
        feedback = evaluator.evaluate_structure_answer(
    ...
    return StructureEvaluationResponse(ai=feedback.as_dict())
...
async def analyze_code_upload(
    ...
    """Analyze a .py/.md upload with rules only; original text is not stored."""
    try:
        return _practice_service.analyze_code_upload(request)
...
async def explain_symbol(
    ...
    """Return a rule-based symbol explanation; no model call."""
    principal_id = principal.principal_id if principal else None
    try:
        return _practice_service.explain_symbol(request, principal_id=principal_id)
    except UploadValidationError as exc:
        raise HTTPException(status_code=exc.status_code, detail=str(exc)) from exc
