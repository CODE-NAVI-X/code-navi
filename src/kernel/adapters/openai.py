...
class OpenAIResponsesAdapter:
    ...
    def _input_items(self, messages: Sequence[Message]) -> list[dict[str, Any]]:
        ...
        for message in messages:
            ...
            for block in message.content:
                if block.type == "text":
                    if message.role == "tool":
                        raise FatalProviderError("OpenAI tool messages cannot contain text blocks")
    ...
    def _tool_use_block(item: Any) -> ContentBlock:
        ...
        try:
            ...
            raise FatalProviderError("OpenAI function call arguments are not valid JSON") from exc
    ...
    def _text_blocks(item: Any) -> list[ContentBlock]:
        ...
        for content in _value(item, "content", ()):
            ...
            if content_type == "output_text":
                ...
                raise FatalProviderError(
                    f"OpenAI message returned unsupported content {content_type!r}"
                )
            if not isinstance(text, str):
