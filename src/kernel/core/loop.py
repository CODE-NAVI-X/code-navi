...
def _inflight_tool_ids(events: Sequence[Event]) -> set[str]:
    called = {event.payload["tool_call"]["id"] for event in events if event.type == "tool_called"}
    returned = {
        event.payload["tool_result"]["tool_call_id"]
        for event in events
...
def run(
    ...
    *,
    ...
    def finish(
        status: RunStatus, reason: str, error: str | None = None, output: Message | None = None
    ) -> RunResult:
    ...
    if recovered_run_id is not None and run_id is not None and run_id != recovered_run_id:
        exc = RuntimeError(
            f"run_id mismatch: prior Events use {recovered_run_id!r}, caller supplied {run_id!r}"
        )
    ...
    while True:
        ...
        try:
            ...
            emit(
                ...
                {
                    "attempt": attempt,
                    "messages": [message.to_json() for message in context_view.messages],
            ...
            emit("error", _error_payload("kernel", "fatal", exc, None))
            return finish(RunStatus.FATAL_ERROR, "context_budget_exceeded", str(exc))
