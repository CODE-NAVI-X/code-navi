...
class ToolSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if any(path not in properties or path not in required for path in paths):
            raise ValueError("workspace path arguments must be declared required schema properties")
...
class PermissionGrant:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "destructive_tool_names", frozenset(self.destructive_tool_names))
...
class ToolRegistry:
    ...
    def register(self, spec: ToolSpec, handler: ToolHandler) -> None:
        ...
        self._entries[spec.name] = _Entry(private_spec, handler, Draft202012Validator(schema))
    ...
    def bind(self, grant: PermissionGrant, context: ToolExecutionContext) -> RunToolDispatcher:
...
class RunToolDispatcher:
    ...
    def dispatch(self, call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            return self._failure(call, "tool_error", str(exc), entry.spec.required_permissions)
        ...
        return ToolResult(
            call.id,
            call.name,
            {"ok": True, "value": value, "audit": audit},
            permissions=entry.spec.required_permissions,
        )

    def _permission_denial(self, spec: ToolSpec, args: Mapping[str, Any]) -> dict[str, Any] | None:
        missing = sorted(
            permission.value
    ...
    def _scope_errors(self, spec: ToolSpec, args: Mapping[str, Any]) -> list[dict[str, str]]:
    ...
    def _failure(
        ...
        result = {
            "ok": False,
            "error": error,
            "audit": {"grant_check": "denied" if code == "permission_denied" else "not_reached"},
        }
        return ToolResult(call.id, call.name, result, f"{code}: {message}", permissions)
