...
class Summarizer(Protocol):
    ...
    def summarize(self, messages: Sequence[Message], budget_tokens: int) -> str: ...
...
class ContextPolicy(Protocol):
    def view(self, events: Sequence[Event], budget_tokens: int) -> ContextView: ...
...
class TailWithSummary:
    ...
    def view(self, events: Sequence[Event], budget_tokens: int) -> ContextView:
        ...
        for cutoff in range(1, len(entries)):
            sources = tuple(entry for entry in entries[:cutoff] if not entry.message.pinned)
            if not sources:
                continue
            retained = tuple(
                entry.message
            ...
            if self._token_counter.count(retained) + self._summary_budget_tokens <= budget_tokens:
        ...
        if exact is not None:
            ...
            if self._token_counter.count((summary_message,)) > self._summary_budget_tokens:
                raise ContextBudgetExceeded("recorded summary exceeds its current token budget")
            messages = self._assemble(entries, cutoff, source_ids, summary)
            if self._token_counter.count(messages) > budget_tokens:
                raise ContextBudgetExceeded("recorded context view exceeds context budget")
        ...
        summary = self._summarizer.summarize(summary_inputs, self._summary_budget_tokens)
